def clean_column(column: list[str]) -> tuple[list[float], list[str], list[str]]:
    """
    Cleans a column consisting of the hourly salary, and extracts the original column, currency, wage, and the salary information into a tuple.
    """
    ...
    return (column, wage, currency, salary_info)